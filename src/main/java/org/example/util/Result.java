...
public final class Result<T> {
    ...
    public <U> Result<U> map(Function<? super T, ? extends U> mapper) {
        Result<U> result = isOk() ? Result.ok(mapper.apply(value)) : Result.error(error);
    ...
    public <U> Result<U> flatMap(Function<? super T, ? extends Result<U>> mapper) {
        Result<U> result = isOk() ? mapper.apply(value) : Result.error(error);
        if (result != null) {
            result.onError(errorConsumer);
            errorConsumer = null;
        }
    ...
    public T orElse(Function<Throwable, T> errorHandler) {
        if (isOk()) {
            return value;
        }
